...
genes_network=data.frame(c(levels(unlist(data.frame(genes_IBD)))))
...
coexpr=fread(paste(args[4],"/coexpr_trial2", sep=""))
...
for(i in 1:nrow(genes_IBD)){ #for each gene pair
  ...
  if((gene1 %in% nodes_func_network)==T & (gene2 %in% nodes_func_network)==T){
    ...
    if(ncol(genes)!=0 & nrow(genes)!=0){
      # retain only genes with a loosely epistatic interaction with at least one other gene on our epistasis networks (??)
...
for (i in 1:length(clusters_allPairs)){ #For each cluster
  for(j in 1:ncol(pathway)){ # and each pathway
    ...
    if(nrow(genesInBoth)>2){
      ...
      output=data.frame(paste(unique(c(as.character(genesInBoth$gene))),collapse=" "), genesInPath[1,1], chi2$p.value, fish$p.value) #and the corresponding P-value
...
pathway_analysis$chi2_correcter_pval=pathway_analysis$chi2.p.value*(ncol(pathway)+length(clusters_allPairs))
pathway_analysis$fish_correcter_pval=pathway_analysis$fish.p.value*(ncol(pathway)+length(clusters_allPairs))
pathway_analysis=pathway_analysis[pathway_analysis$chi2_correcter_pval<=0.05,]
pathway_analysis=pathway_analysis[pathway_analysis$fish_correcter_pval<=0.05,]
pathway_analysis=pathway_analysis[order(pathway_analysis$fish_correcter_pval),]
